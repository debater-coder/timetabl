...
// Error Table:
// 0x01: The cookies are invalid
// 0x02: The cookies are missing
// 0x03: Refresh the access token.
// 0x04: No access token or refresh token
...
const fetch_api = (endpoint, parameters = {}, event, authenticated = false) => {
  if (!authenticated) {
    ...
    if (!event.headers.cookie) {
      throw new AuthenticationError('0x02: The cookies are missing');
    }
    let cookies;
    ...
    try {
      ...
      throw new AuthenticationError('0x01: The cookies are invalid');
    }
    ...
    if (cookies['access_token']) {
      return fetch('https://student.sbhs.net.au/api/' + endpoint + '?' + new URLSearchParams(parameters) +
        ...
        .then((res) => {
          if (!res.ok) {
            if (res.status === 401) {
              throw new AuthenticationError('0x03: Refresh the access token.');
            }
      ...
      throw new AuthenticationError('0x04: No access token or refresh token');
    }
  }
};
